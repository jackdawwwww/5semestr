...
public class Discoverer implements Runnable {
    ...
    private final HashMap<Node, Long> lastUpdate;

    private final ConcurrentHashMap<Node, SnakesProto.GameMessage.AnnouncementMsg> sessionInfoMap;

    public Discoverer(ConcurrentHashMap<Node, SnakesProto.GameMessage.AnnouncementMsg> _sessionInfoMap) throws IOException {
        ...
        try {
            ...
            for(SnakesProto.GamePlayer player : announcementMsg.getPlayers().getPlayersList()) {
                if(player.getRole() == SnakesProto.NodeRole.MASTER) {
                    hi = new Node(dp.getAddress(), player.getPort());
                }
            }
